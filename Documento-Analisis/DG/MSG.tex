\chapter{Mensajes}

% -------- INCLUIDO POR : ABIGAIL NICOLAS
\subsection{MSG0. Matrícula y/o contraseña no válidas.}
    %Descripción del mensaje, cuándo aparece y en que IU se muestra
    %Cómo se cierra ese msj
    % Imagen del msj
    \begin{figure}[htbp]
        \begin{center}
            \includegraphics[width=.4\textwidth]{mensajes/MSGT0}
            \caption{MSG0 Matrícula y/o contraseña no válidas.}
            \label{fig:MSG0}
        \end{center}
    \end{figure}
    
% -------- INCLUIDO POR : 
\subsection{MSG1. ¿Seguro que desea editar el Programa Académico?}
    %Descripción del mensaje, cuándo aparece y en que IU se muestra
    %Cómo se cierra ese msj
    % Imagen del msj
    \begin{figure}[htbp]
        \begin{center}
            \includegraphics[width=.4\textwidth]{mensajes/MSGT1}
            \caption{MSG1 ¿Seguro que desea editar el Programa Académico?}
            \label{fig:MSG1}
        \end{center}
    \end{figure}
    
% -------- INCLUIDO POR : 
\subsection{MSG2. ¿Seguro que desea cancelar la consulta de Programas Académicos?}
    %Descripción del mensaje, cuándo aparece y en que IU se muestra
    %Cómo se cierra ese msj
    % Imagen del msj
    \begin{figure}[htbp]
        \begin{center}
            \includegraphics[width=.4\textwidth]{mensajes/MSGT2}
            \caption{MSG2 ¿Seguro que desea cancelar la consulta de Programas Académicos?}
            \label{fig:MSG2}
        \end{center}
    \end{figure}
    
% -------- INCLUIDO POR : 
\subsection{MSG3. ¿Seguro que desea registrar un Programa Académico?}
    %Descripción del mensaje, cuándo aparece y en que IU se muestra
    %Cómo se cierra ese msj
    % Imagen del msj
    \begin{figure}[htbp]
        \begin{center}
            \includegraphics[width=.4\textwidth]{mensajes/MSGT3}
            \caption{MSG3 ¿Seguro que desea registrar un Programa Académico?}
            \label{fig:MSG3}
        \end{center}
    \end{figure}

% -------- INCLUIDO POR : ARTURO, CINTHYA, NAIBI
\subsection{MSG4. Los campos marcados con (*) son obligatorios.}
    %Descripción del mensaje, cuándo aparece y en que IU se muestra
    %Cómo se cierra ese msj
    % Imagen del msj
    \begin{figure}[htbp]
        \begin{center}
            \includegraphics[width=.4\textwidth]{mensajes/MSGT4}
            \caption{MSG4 Los campos marcados con (*) son obligatorios.}
            \label{fig:MSG4}
        \end{center}
    \end{figure}

% -------- INCLUIDO POR : ARTURO, NAIBI, CINTHYA
\subsection{MSG5. Registro finalizado exitosamente.}
    %Descripción del mensaje, cuándo aparece y en que IU se muestra
    %Cómo se cierra ese msj
    % Imagen del msj
    \begin{figure}[htbp]
        \begin{center}
            \includegraphics[width=.4\textwidth]{mensajes/MSGT5}
            \caption{MSG5 Registro finalizado exitosamente.}
            \label{fig:MSG5}
        \end{center}
    \end{figure}

% -------- INCLUIDO POR : ARTURO, NAIBI
\subsection{MSG6. ¿Seguro que desea cancelar el registro?}
    %Descripción del mensaje, cuándo aparece y en que IU se muestra
    %Cómo se cierra ese msj
    % Imagen del msj
    \begin{figure}[htbp]
        \begin{center}
            \includegraphics[width=.4\textwidth]{mensajes/MSGT6}
            \caption{MSG6 ¿Seguro que desea cancelar?}
            \label{fig:MSG6}
        \end{center}
    \end{figure}

% -------- INCLUIDO POR : ARTURO, NAIBI
\subsection{MSG7. Los catálogos necesarios no se han cargado, favor de intentarlo más tarde.}
    %Descripción del mensaje, cuándo aparece y en que IU se muestra
    %Cómo se cierra ese msj
    % Imagen del msj
    \begin{figure}[htbp]
        \begin{center}
            \includegraphics[width=.4\textwidth]{mensajes/MSGT7}
            \caption{MSG7 Los catálogos necesarios no se han cargado, favor de intentarlo más tarde.}
            \label{fig:MSG7}
        \end{center}
    \end{figure}
    
% -------- INCLUIDO POR : ARTURO, NAIBI
\subsection{MSG8. El libro con el ISBN "Número de ISBN" ya existe.}
    %Descripción del mensaje, cuándo aparece y en que IU se muestra
    %Cómo se cierra ese msj
    % Imagen del msj
    \begin{figure}[htbp]
        \begin{center}
            \includegraphics[width=.4\textwidth]{mensajes/MSGT8}
            \caption{MSG8 El libro con el ISBN "Número de ISBN" ya existe.}
            \label{fig:MSG8}
        \end{center}
    \end{figure}

% -------- INCLUIDO POR : ARTURO, NAIBI
\subsection{MSG9. Por el momento no se puede realizar el registro.}
    %Descripción del mensaje, cuándo aparece y en que IU se muestra
    %Cómo se cierra ese msj
    % Imagen del msj
    \begin{figure}[htbp]
        \begin{center}
            \includegraphics[width=.4\textwidth]{mensajes/MSGT9}
            \caption{MSG9 Por el momento no se puede realizar el registro.}
            \label{fig:MSG9}
        \end{center}
    \end{figure}

% -------- INCLUIDO POR : CINTHYA
\subsection{MSG10. Error: Primero debe seleccionar el punto en donde se agregará el comentario.}
    %Descripción del mensaje, cuándo aparece y en que IU se muestra
    %Cómo se cierra ese msj
    % Imagen del msj
    \begin{figure}[htbp]
        \begin{center}
            \includegraphics[width=.4\textwidth]{mensajes/MSGT10}
            \caption{MSG10 Error: Primero debe seleccionar el punto en donde se agregará el comentario.}
            \label{fig:MSG10}
        \end{center}
    \end{figure}

% -------- INCLUIDO POR : CINTHYA
\subsection{MSG11. Error: Debe agregar un texto al nuevo comentario.}
    %Descripción del mensaje, cuándo aparece y en que IU se muestra
    %Cómo se cierra ese msj
    % Imagen del msj
    \begin{figure}[htbp]
        \begin{center}
            \includegraphics[width=.4\textwidth]{mensajes/MSGT11}
            \caption{MSG11 Error: Debe agregar un texto al nuevo comentario.}
            \label{fig:MSG11}
        \end{center}
    \end{figure}

% -------- INCLUIDO POR : CINTHYA
\subsection{MSG12. Ingrese su comentario en el nuevo campo de la bitácora.}
    %Descripción del mensaje, cuándo aparece y en que IU se muestra
    %Cómo se cierra ese msj
    % Imagen del msj
    \begin{figure}[htbp]
        \begin{center}
            \includegraphics[width=.4\textwidth]{mensajes/MSGT12}
            \caption{MSG12 Ingrese su comentario en el nuevo campo de la bitácora.}
            \label{fig:MSG12}
        \end{center}
    \end{figure}

% -------- INCLUIDO POR : CINTHYA
\subsection{MSG13. Revisión de paquete finalizada. Paquete enviado.}
    %Descripción del mensaje, cuándo aparece y en que IU se muestra
    %Cómo se cierra ese msj
    % Imagen del msj
    \begin{figure}[htbp]
        \begin{center}
            \includegraphics[width=.4\textwidth]{mensajes/MSGT13}
            \caption{MSG13 Revisión de paquete finalizada. Paquete enviado.}
            \label{fig:MSG13}
        \end{center}
    \end{figure}

% -------- INCLUIDO POR : CINTHYA
\subsection{MSG14. Error al finalizar paquete.}
    %Descripción del mensaje, cuándo aparece y en que IU se muestra
    %Cómo se cierra ese msj
    % Imagen del msj
    \begin{figure}[htbp]
        \begin{center}
            \includegraphics[width=.4\textwidth]{mensajes/MSGT14}
            \caption{MSG14 Error al finalizar paquete.}
            \label{fig:MSG14}
        \end{center}
    \end{figure}

% -------- INCLUIDO POR : CINTHYA
\subsection{MSG15 ¿Está seguro que desea cancelar la revisión? Se perderán las anotaciones que no hayas guardado anteriormente.}
    %Descripción del mensaje, cuándo aparece y en que IU se muestra
    %Cómo se cierra ese msj
    % Imagen del msj
    \begin{figure}[htbp]
        \begin{center}
            \includegraphics[width=.4\textwidth]{mensajes/MSGT15}
            \caption{MSG15 ¿Está seguro que desea cancelar la revisión? Se perderán las anotaciones que no hayas guardado anteriormente.}
            \label{fig:MSG15}
        \end{center}
    \end{figure}

% -------- INCLUIDO POR : CINTHYA
\subsection{MSG16. ¿Está seguro de finalizar la revisión?}
    %Descripción del mensaje, cuándo aparece y en que IU se muestra
    %Cómo se cierra ese msj
    % Imagen del msj
    \begin{figure}[htbp]
        \begin{center}
            \includegraphics[width=.4\textwidth]{mensajes/MSGT16}
            \caption{MSG16 ¿Está seguro de finalizar la revisión?}
            \label{fig:MSG16}
        \end{center}
    \end{figure}

% -------- INCLUIDO POR : CINTHYA
\subsection{MSG17. Sección Aprobada}
    %Descripción del mensaje, cuándo aparece y en que IU se muestra
    %Cómo se cierra ese msj
    % Imagen del msj
    \begin{figure}[htbp]
        \begin{center}
            \includegraphics[width=.4\textwidth]{mensajes/MSGT17}
            \caption{MSG17 Sección Aprobada}
            \label{fig:MSG17}
        \end{center}
    \end{figure}
    

% -------- INCLUIDO POR : CINTHYA
\subsection{MSG18. Revisión de Sección Finalizada.}
    %Descripción del mensaje, cuándo aparece y en que IU se muestra
    %Cómo se cierra ese msj
    % Imagen del msj
    \begin{figure}[htbp]
        \begin{center}
            \includegraphics[width=.4\textwidth]{mensajes/MSGT18}
            \caption{MSG18 Revisión de Sección Finalizada.}
            \label{fig:MSG18}
        \end{center}
    \end{figure}
    
% -------- INCLUIDO POR : CINTHYA
\subsection{MSG19. Anotaciones en sección guardadas correctamente.}
    %Descripción del mensaje, cuándo aparece y en que IU se muestra
    %Cómo se cierra ese msj
    % Imagen del msj
    \begin{figure}[htbp]
        \begin{center}
            \includegraphics[width=.4\textwidth]{mensajes/MSGT19}
            \caption{MSG19 Anotaciones en sección guardadas correctamente.}
            \label{fig:MSG19}
        \end{center}
    \end{figure}

% -------- INCLUIDO POR : CINTHYA
\subsection{MSG20. Los campos no fueron contestados correctamente.}
    %Descripción del mensaje, cuándo aparece y en que IU se muestra
    %Cómo se cierra ese msj
    % Imagen del msj
    \begin{figure}[htbp]
        \begin{center}
            \includegraphics[width=.4\textwidth]{mensajes/MSGT20}
            \caption{MSG20 Los campos no fueron contestados correctamente.}
            \label{fig:MSG20}
        \end{center}
    \end{figure}

% -------- INCLUIDO POR : NAIBI (CU1 )
\subsection{MSG21. No hay usuarios registrados con ese cargo.}
    %Descripción del mensaje, cuándo aparece y en que IU se muestra
    %Cómo se cierra ese msj
    % Imagen del msj
    \begin{figure}[htbp]
        \begin{center}
            \includegraphics[width=.4\textwidth]{mensajes/MSGT21}
            \caption{MSG21 No hay usuarios registrados con ese cargo.}
            \label{fig:MSG21}
        \end{center}
    \end{figure}


% -------- INCLUIDO POR : NAIBI ( CU1 )
\subsection{MSG22. ¿Seguro de eliminar al empleado ''Nombre'' con matrícula: ''Matrícula'' del sistema?}
    %Descripción del mensaje, cuándo aparece y en que IU se muestra
    %Cómo se cierra ese msj
    % Imagen del msj
    \begin{figure}[htbp]
        \begin{center}
            \includegraphics[width=.4\textwidth]{mensajes/MSGT22}
            \caption{MSG22 ¿Seguro de eliminar al empleado ''Nombre'' con matrícula: ''Matrícula'' del sistema?}
            \label{fig:MSG22}
        \end{center}
    \end{figure}
    
% -------- INCLUIDO POR : NAIBI
\subsection{MSG23. Los porcentajes de evaluación no cumplen con el porcentaje total.}
    %Descripción del mensaje, cuándo aparece y en que IU se muestra
    %Cómo se cierra ese msj
    % Imagen del msj
    % Se excedio el porcentaje total o aún no se llega a éste.
    \begin{figure}[htbp]
        \begin{center}
            \includegraphics[width=.4\textwidth]{mensajes/MSGT23}
            \caption{ MSG23 Los porcentajes de evaluación no cumplen con el porcentaje total.}
            \label{fig:MSG21}
        \end{center}
    \end{figure}

% -------- INCLUIDO POR : ENRIQUE, ABI
\subsection{MSG24. Por el momento la página solicitada no esta disponible.}
    %Descripción del mensaje, cuándo aparece y en que IU se muestra
    %Cómo se cierra ese msj
    % Imagen del msj
    % El sistema no encuentra la interfaz a mostrar o existe un problema con el servidor.
    \begin{figure}[htbp]
        \begin{center}
            \includegraphics[width=.4\textwidth]{mensajes/MSGT24}
            \caption{MSG24 Por el momento la página solicitada no esta disponible.}
            \label{fig:MSG24}
        \end{center}
    \end{figure}

% -------- INCLUIDO POR : ENRIQUE, ABI
\subsection{MSG25. Servicios no disponibles por el momento.}
    %Descripción del mensaje, cuándo aparece y en que IU se muestra
    %Cómo se cierra ese msj
    % Imagen del msj
    % La base de datos no está disponible, o hubo un error de conexión.
    \begin{figure}[htbp]
        \begin{center}
            \includegraphics[width=.4\textwidth]{mensajes/MSGT25}
            \caption{MSG25 Servicios no disponibles por el momento.}
            \label{fig:MSG25}
        \end{center}
    \end{figure}

% -------- INCLUIDO POR : ENRIQUE, ABI
\subsection{MSG26. La matrícula ingresada no se encuentra registrada en el sistema.}
    %Descripción del mensaje, cuándo aparece y en que IU se muestra
    %Cómo se cierra ese msj
    % Imagen del msj
    % La matrícula ingresada no se encuentra registrada en el sistema.
    \begin{figure}[htbp]
        \begin{center}
            \includegraphics[width=.4\textwidth]{mensajes/MSGT26}
            \caption{MSG26 La matrícula ingresada no se encuentra registrada en el sistema.}
            \label{fig:MSG26}
        \end{center}
    \end{figure}

% -------- INCLUIDO POR : ENRIQUE, RAFA
\subsection{MSG27. Empleado modificado.}
    %Descripción del mensaje, cuándo aparece y en que IU se muestra
    %Cómo se cierra ese msj
    % Imagen del msj
    % Se han editado los datos de un empleado
    \begin{figure}[htbp]
        \begin{center}
            \includegraphics[width=.4\textwidth]{mensajes/MSGT27}
            \caption{MSG27 Empleado modificado.}
            \label{fig:MSG27}
        \end{center}
    \end{figure}
    
% -------- INCLUIDO POR : NAIBI
\subsection{MSG28. El autor: "Nombre Primer Apellido Segundo Apellido" ya existe.}
    %Descripción del mensaje, cuándo aparece y en que IU se muestra
    %Cómo se cierra ese msj
    % Imagen del msj
    % Se han editado los datos de un empleado
    \begin{figure}[htbp]
        \begin{center}
            \includegraphics[width=.4\textwidth]{mensajes/MSGT28}
            \caption{MSG28 El autor: "Nombre Primer Apellido Segundo Apellido" ya existe.}
            \label{fig:MSG28}
        \end{center}
    \end{figure}    

% -------- INCLUIDO POR : RAFA, ENRIQUE
\subsection{MSG29. El usuario se ha registrado exitosamente.}
    %Descripción del mensaje, cuándo aparece y en que IU se muestra
    %Cómo se cierra ese msj
    % Imagen del msj
    % Se han editado los datos de un empleado
    \begin{figure}[htbp]
        \begin{center}
            \includegraphics[width=.4\textwidth]{mensajes/MSGT29}
            \caption{MSG29. El usuario se ha registrado exitosamente.}
            \label{fig:MSG29}
        \end{center}
    \end{figure}  

% -------- INCLUIDO POR : RAFA, ENRIQUE
\subsection{MSG30. Error, el usuario ya existe en el sistema.}
    %Descripción del mensaje, cuándo aparece y en que IU se muestra
    %Cómo se cierra ese msj
    % Imagen del msj
    % Se han editado los datos de un empleado
    \begin{figure}[htbp]
        \begin{center}
            \includegraphics[width=.4\textwidth]{mensajes/MSGT30}
            \caption{MSG30. Error, el usuario ya existe en el sistema.}
            \label{fig:MSG30}
        \end{center}
    \end{figure} 

% -------- INCLUIDO POR : RAFA, ENRIQUE
\subsection{MSG31. Inconsistencia en los datos. Verifique los campos e intente de nuevo.}
    %Descripción del mensaje, cuándo aparece y en que IU se muestra
    %Cómo se cierra ese msj
    % Imagen del msj
    % Se han editado los datos de un empleado
    \begin{figure}[htbp]
        \begin{center}
            \includegraphics[width=.4\textwidth]{mensajes/MSGT31}
            \caption{MSG31. Inconsistencia en los datos. Verifique los campos e intente de nuevo.}
            \label{fig:MSG31}
        \end{center}
    \end{figure}

% -------- INCLUIDO POR : RAFA, ENRIQUE
\subsection{MSG32. No existen recursos humanos registrados en el sistema con la matrícula ingresada.}
    %Descripción del mensaje, cuándo aparece y en que IU se muestra
    %Cómo se cierra ese msj
    % Imagen del msj
    % Se han editado los datos de un empleado
    \begin{figure}[htbp]
        \begin{center}
            \includegraphics[width=.4\textwidth]{mensajes/MSGT32}
            \caption{MSG32. No existen recursos humanos registrados en el sistema con la matrícula ingresada.}
            \label{fig:MSG32}
        \end{center}
    \end{figure}  
    
    

