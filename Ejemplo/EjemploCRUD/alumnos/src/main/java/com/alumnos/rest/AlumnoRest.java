...
public class AlumnoRest {
	...
	@GetMapping(path="/obtener") //Esta función es para refrescar el select del ejemplo alumno.
	public @ResponseBody ArrayList<String> buscarAlumnos() {
		Iterable<Alumno> alumnosAux = alumnoRepo.findAll();
		ArrayList<String> alumnos = new ArrayList<String>();
		for(Alumno s : alumnosAux) {
			alumnos.add(s.toString());
		}
		return alumnos;		
	}
	
	@PostMapping(path="/eliminar")
	public @ResponseBody boolean eliminarAlumno(@RequestBody Map<String, Object> datos) {
		Alumno alumno = null;
		if(datos.get("alumnos")!=null) {
			ArrayList<?> idsAlumnos = (ArrayList<?>) datos.get("alumnos"); //Hacemos un casteo, debido a que este es un tipo de dato array, enviado por
			for(Object id : idsAlumnos) {
				alumno = alumnoRepo.findByIds((Integer.parseInt((String)id))); //crear esta clase para obtener un Objeto tipo Materia
			}
		}
	...
	public @ResponseBody boolean modificarAlumno(@RequestBody Map<String, Object> datos) {
		int idA = 0;
		if(datos.get("alumnos")!=null) {
			ArrayList<?> idsAlumnos = (ArrayList<?>) datos.get("alumnos"); //Hacemos un casteo, debido a que este es un tipo de dato array, enviado por
			for(Object id : idsAlumnos) {
				idA = Integer.parseInt((String)id);
			}
		}
		alumnoRepo.setUserInfo(idA, Integer.toString((Integer)datos.get("boleta")));
		return true;
	}
